...
7 个真实数据源并行扫描刚开始升温的话题,在热搜出现前 7-14 天发现机会。
...
\textbf{7 个真实数据源并行}(全部真实 API,零 Mock):

\textbf{国际信号层}:
\begin{enumerate}
\item Polymarket(预测市场,真金白银 24h 交易量,gamma-api.polymarket.com)
\item Manifold Markets(社区预测市场,manifold.markets API)
\item HackerNews(科技/创业叙事先行,hacker-news.firebaseio.com)
\end{enumerate}

\textbf{国内热搜层}(via xxapi.cn 聚合 API):
\begin{enumerate}[resume]
\item 微博热搜(舆论风向标,有热度值如「790万」)
\item 抖音热搜(短视频生态)
\item 百度热搜(搜索意图,有热度值)
\item B站热门(年轻用户 + 深度内容)
\end{enumerate}
...
\node[layer, below=2cm of p2, xshift=2cm] (models) {\textbf{LLM 池}: MiMo 首选 / MiniMax 备用 / 混元兜底 / BYOK / 本地 Ollama};
...
LLM & 小米 MiMo-V2.5-Pro 首选 + MiniMax M2.7 备用 + 腾讯混元兜底 + BYOK(DeepSeek/豆包/智谱/Kimi/Claude/GPT/Ollama) \\
...
\item 点击「今日早期信号」,Ripple 7 个真实数据源并行扫描,2 秒返回 Top 10 趋势
