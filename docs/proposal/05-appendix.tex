...
\subsubsection{Oracle 早期信号雷达 v2 - 7 真实数据源并行}

\begin{lstlisting}[language=python, caption=apps/api/agents/oracle\_agent.py(节选)]
class OracleAgent:
    def __init__(self):
        self.sources = [
            PolymarketSource(),    # Polymarket 预测市场 (真实交易量)
            ManifoldSource(),      # Manifold 社区预测
            HackerNewsSource(),    # HackerNews 科技叙事
            WeiboHotSource(),      # 微博热搜 (xxapi.cn 真实数据)
            DouyinHotSource(),     # 抖音热搜 (xxapi.cn 真实数据)
            BaiduHotSource(),      # 百度热搜 (xxapi.cn 真实数据)
            BilibiliHotSource(),   # B站热门 (xxapi.cn 真实数据)
        ]

    async def scan(self, topic_seed, category, target_platforms):
        # 7 真实数据源并行扫描
        hotlists = {}
        tasks = [(s.name, s.fetch_hotlist()) for s in self.sources]
        results = await asyncio.gather(
            *[t[1] for t in tasks], return_exceptions=True
        )

        # 跨平台矩阵比对 + 矛盾推理
        contradictions = find_contradictions(hotlists, topic_keywords)

        # CUSUM + MAD-zscore 聚合趋势 + 置信度
        trends = self._aggregate_trends(
            topic_seed, topic_keywords, hotlists,
            all_samples, contradictions, category, target_platforms
        )
