...
### 赛道机会评分：X/10
用 1-10 分评估这个赛道/方向，必须说明：
- 具体数据支撑（搜索量、竞品数量、增长率等，从搜索数据中提取）
- 竞争程度分析
- 为什么现在是好时机

### 内容策略
- 差异化定位建议（2-3 条，必须具体到执行层面）
- 推荐的内容形式和风格（说明为什么适合这个赛道）
- 5 个标题建议（附预估CTR百分比，格式如"标题内容 (预估CTR: 12%)"）
- 3 个开场 Hook（附预估留存提升百分比，格式如"Hook内容" (+35%留存)）

### 微信生态策略
这是核心差异化部分，必须非常具体：
- **视频号**：具体发布时间（精确到星期几和小时）、关键词列表（至少5个）、时长建议、完播率优化技巧
- **搜一搜**：基于Peoplerank算法（账号可信度25% + 内容相关性40% + 用户行为35%），给出核心词+长尾词+蓝海词各3-5个，附月搜索量预估
- **公众号**：内容格式、更新频率、SEO关键词布局建议

### 30天行动路径
按周给出具体可执行的计划，每周必须包含：
- 具体要做什么（发多少条、什么平台、什么形式）
- 量化目标（粉丝数、互动率、发布量）
- 关键动作（如"第X天做XX"）

格式：
**D1-D7（XX期）：** 具体内容... 目标：XXX
**D8-D14（XX期）：** 具体内容... 目标：XXX
**D15-D21（XX期）：** 具体内容... 目标：XXX
**D22-D30（XX期）：** 具体内容... 目标：XXX
...
**硬性要求**：
- 所有观点必须基于搜索数据，不编造数字
- 在引用搜索数据中的信息时，使用[编号]格式标注来源，如"根据数据[1]..."、"某平台显示[3]..."
- 必须包含具体数字（搜索量、CTR、时间等），不能只给模糊建议
- 策略必须可执行（大学生能独立完成的级别）
- 语气亲和专业，像学长/学姐分享经验
- 如果数据不足明确说明"数据有限，以下为推测"
- 内容必须与微信/视频号/搜一搜/公众号生态紧密结合"""
    ...
    # Generate follow-up suggestions based on the domain
    follow_ups = [
        {"label": f"📊 {domain}的竞品有哪些？", "prompt": f"帮我分析{domain}领域的竞品账号"},
        {"label": "✍️ 帮我写一篇笔记", "prompt": f"根据上面的分析，帮我写一篇关于{domain}的小红书爆款笔记"},
        {"label": "🎯 如何做差异化？", "prompt": f"在{domain}领域，如何做出差异化定位？"},
        {"label": "📈 下一步怎么做？", "prompt": f"基于上面的30天计划，第一周我具体应该怎么执行？"},
    ]
    yield sse_event("next_steps", follow_ups)
