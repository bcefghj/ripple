...
可选 intent 类型：
- "radar": 用户想了解某个领域/话题的内容生态、趋势分析、增长机会
- "idea": 用户想要选题灵感、话题推荐
...
判断规则（重要：优先判断为 radar）：
- 如果用户输入了一个话题/领域/事件/关键词，即使没有明确说"分析" → radar
- 如果用户提到"领域""行业""同行""博主""达人""分析""趋势" → radar
- 如果用户提到"选题""灵感""话题""点子""想做什么内容" → idea
- 如果用户提到"能火吗""爆款""评估""预测""这个标题怎么样" → predict
- 如果用户明确说"写""创作""文案""帮我出一篇" → create
- 如果用户提到"风格""蒸馏""写作方法""学习某博主" → distill
- **绝大多数情况应返回 "radar"**，只有在用户明确要求其他操作时才选其他类型
- domain：从用户输入中提取领域/话题关键词，没有就用原始消息
